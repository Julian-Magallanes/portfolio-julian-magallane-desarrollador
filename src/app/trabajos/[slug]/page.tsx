  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-10">
        <div>
            ...
            <p className="text-xl font-regular">{job[0].description}</p>
            <div className="flex gap-5">
            {job[0].urlPage && <Button title="Ver sitio web" color="bg-primary" onClick={job[0].urlPage} iconLeft={<FontAwesomeIcon icon={faArrowPointer} style={{width:"20px", height:"20px"}}/>}/>}
            {job[0].urlGitHub && <Button title="Ver repositorio" color="bg-primary" onClick={job[0].urlGitHub} iconLeft={<FontAwesomeIcon icon={faGithub} style={{width:"20px", height:"20px"}}/>}/>}
            </div>
            {job[0].urlUIKit && 
            job[0].urlUIKit.map ((url, index) => 
                <Image key={index} src={url} width={1000} height={1000} alt={job[0].title}/>
            )}
            ...
            {job[0].urlPrototype && <Image src={job[0].urlPrototype} width={1000} height={1000} alt={job[0].title}/>}
            <h2 className="text-2xl font-medium">Tecnologias utilizadas</h2>
